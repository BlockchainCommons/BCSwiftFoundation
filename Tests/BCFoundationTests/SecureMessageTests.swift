...
class SecureMessageTests: XCTestCase {
    ...
    static let additionalData = Data(hex: "50515253c0c1c2c3c4c5c6c7")!
    ...
    static let secureMessage = SecureMessage(plaintext: plaintext, additionalData: additionalData, key: key, nonce: nonce)!
    ...
    static let authenticationTag = SecureMessage.AuthenticationTag(Data(hex: "1ae10b594f09e26a7e902ecbd0600691")!)!

    func testRFVTestVector() throws {
        XCTAssertEqual(Self.secureMessage.ciphertext, Self.ciphertext)
        XCTAssertEqual(Self.secureMessage.authenticationTag, Self.authenticationTag)

        let decryptedSecureMessage = SecureMessage(ciphertext: Self.ciphertext, additionalData: Self.additionalData, key: Self.key, nonce: Self.nonce, authenticationTag: Self.secureMessage.authenticationTag)!
    ...
    func testRandomKeyAndNonce() {
        ...
        let secureMessage = SecureMessage(plaintext: Self.plaintext, additionalData: Self.additionalData, key: key, nonce: nonce)!
        let decryptedSecureMessage = SecureMessage(ciphertext: secureMessage.ciphertext, additionalData: Self.additionalData, key: key, nonce: nonce, authenticationTag: secureMessage.authenticationTag)!
    ...
    func testEmptyData() {
        let key = SecureMessage.Key()
        let nonce = SecureMessage.Nonce()
        let secureMessage = SecureMessage(plaintext: Data(), additionalData: Data(), key: key, nonce: nonce)!
        let decryptedSecureMessage = SecureMessage(ciphertext: secureMessage.ciphertext, additionalData: Data(), key: key, nonce: nonce, authenticationTag: secureMessage.authenticationTag)!
        XCTAssertEqual(secureMessage, decryptedSecureMessage)
    }
