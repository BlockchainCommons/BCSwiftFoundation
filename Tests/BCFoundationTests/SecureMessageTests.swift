...
class SecureMessageTests: XCTestCase {
    ...
    static let key = SecureMessage.Key(rawValue: Data(hex: "808182838485868788898a8b8c8d8e8f909192939495969798999a9b9c9d9e9f")!)!
    static let nonce = SecureMessage.Nonce(rawValue: Data(hex: "070000004041424344454647")!)!
    static let secureMessage = SecureMessage(plaintext: plaintext, key: key, aad: aad, nonce: nonce)
    ...
    static let auth = SecureMessage.Auth(rawValue: Data(hex: "1ae10b594f09e26a7e902ecbd0600691")!)!
    ...
    func testRandomKeyAndNonce() {
        ...
        let secureMessage = SecureMessage(plaintext: Self.plaintext, key: key, aad: Self.aad, nonce: nonce)
    ...
    func testEmptyData() {
        let key = SecureMessage.Key()
        let nonce = SecureMessage.Nonce()
        let secureMessage = SecureMessage(plaintext: Data(), key: key, aad: Data(), nonce: nonce)
        let decryptedPlaintext = key.decrypt(message: secureMessage)
